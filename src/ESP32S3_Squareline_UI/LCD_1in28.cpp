...
static void LCD_1IN28_SetAttributes(UBYTE Scan_dir)
{
    ...
    if(Scan_dir == HORIZONTAL) {
        ...
        // Try different MADCTL values if UI is misaligned:
        // 0x08 = BGR only (no flip)
        // 0x48 = MX + BGR (mirror X)
        // 0x88 = MY + BGR (mirror Y)
        // 0xC8 = MX+MY + BGR (mirror both) - original value
        MemoryAccessReg = 0x48;  // Try MX+BGR (mirror X)
    } else {
...
void LCD_1IN28_SetWindows(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend)
{
	...
	LCD_1IN28_SendData_8Bit((Yend-1)>>8);  // FIXED: Was (Xend-1)>>8
